...
| Root/child completeness | Stable totals, bounded page count, short/empty/no-unique-progress rejection, cross-page duplicate handling, root union, child-subset checks, root-only/new-child `UNKNOWN`, immutable per-root and per-child reports. |
...
| Source-safe client | Explicit connect/read/call timeouts, decompressed response limit, content-type check, global rate/concurrency gate, full-jitter retry, both `Retry-After` forms, bounded attempts, contact-bearing User-Agent, shutdown cancellation. |
| Durable orchestration | One named queue-free Spring worker, optional managed schedule, PostgreSQL claim/advisory lock, idempotent UUID trigger, stale-run recovery, retained progress/retry/error coordinates, late recovered-task refusal. |
| Atomic publication | V10 exact root/direct-child success gates; candidate-count and membership-scope checks; one transaction for auctions, memberships, observations, absences, `SUCCEEDED`, and enrichment queue. Missing, partial, non-subset, or failed detail evidence rolls back without current-state mutation. |
| API and UI | Loopback-only `POST /api/sync/runs`; no-store `202`/`200` replay semantics and structured `400`/`403`/`404`/`409`/`503`; retained root/child/error status; one UI trigger with bounded polling and terminal reset. |
...
`postgis/postgis:18-3.6`, Flyway V1 through V10, and the pinned Playwright
Chromium runtime.
...
51 suites; 267 tests; 263 passed; 4 skipped; 0 failures; 0 errors

./gradlew clean check --no-daemon
BUILD SUCCESSFUL in 1m 4s
same 267 Java/PostgreSQL tests, plus 14/14 offline basemap contracts
