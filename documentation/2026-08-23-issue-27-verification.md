...
| Contract | Evidence |
|---|---|
| Local basemap and bounded viewport loading | `auction-map.mjs` creates the #25 same-origin PMTiles map and requests only `/api/map/auctions` with its current WGS84 bounds, a `1000` limit, and allowlisted filters. A pending `moveend` refresh has a 250 ms fallback and may run earlier on settled `idle` (`≤250 ms`); scheduling another load aborts the active `AbortController`. Pitch and rotation are disabled while pan, zoom, and pinch zoom remain available. Minimum zoom is calculated from actual canvas dimensions against the API's spherical area ceiling, recalculated on resize, and guarded again before fetch. |
...
| Offline proof and evidence | Every #27 browser case reuses `LocalhostOnlyNetwork` and finishes with contacted hosts exactly `[localhost]`, no blocked host, and no public map asset request. The complete-flow case writes `issue-27-auction-map-desktop.png`, `issue-27-auction-map-narrow.png`, and `issue-27-auction-map.json` under `build/browser-test-results/evidence/`. Production pages do not expose the browser-only `window.__auctionMap` hook. |
...
### 2026-08-24 follow-up remediation

1. The auction-map consumer requests a zero-pitch/zero-bearing 2D camera and
   disables mouse rotation, rotate-to-pitch, touch pitch, touch rotation, and
   keyboard rotation without removing pinch zoom. The shared #25 local-basemap
   factory stays policy-neutral, while #27's area guard operates on the same
   camera model it estimates.
2. A precision-catalog mismatch stops before basemap initialization and names
   the application version contract, not the PMTiles bundle.
3. Runtime map errors are classified as basemap, auction-source, or generic
   resource failures, including recoverable errors emitted before initial
   `load`. Source-attributed warnings clear only when that exact source reports
   a successful loaded state. Keyless warnings remain visible until reload;
   unrelated `styledata` is not treated as recovery evidence.
4. Initial style/basemap-source definition failure rejects immediately;
   individual tile errors remain recoverable and the 30-second timeout remains
   only as the final no-event safety bound.
5. The RSD formatter is module-scoped. The 160-result browser fixture proves a
   single formatter construction rather than one construction per feature.
   Documentation now describes the measured refresh boundary as `≤250 ms`,
   because `idle` can beat the fallback timer while rapid changes still
   coalesce.
...
AuctionMapBrowserTest > precisionContractFailureNamesTheApplicationContractNotTheBasemap() PASSED
AuctionMapBrowserTest > mapResourceErrorsAreClassifiedAndClearWhenTheirSourceRecovers() PASSED
AuctionMapBrowserTest > initialStyleFailureRejectsImmediatelyButTileFailureRemainsRecoverable() PASSED
AuctionMapBrowserTest > manyViewportResultsScrollWithoutChangingDesktopMapHeight() PASSED
BUILD SUCCESSFUL
```

The suite uses the production HTTP, PostGIS, GeoJSON, PMTiles, MapLibre, URL,
popup, accessibility, and responsive paths. Controlled boundaries replace only
the viewport response, one precision option, formatter constructor, or map
events needed to make delayed/aborted, style-deferred, contract-failure,
resource-recovery, and fast-failure behavior deterministic. The wide-camera
case uses the production API at its responsive minimum. None can reach a
non-loopback host.
...
browserTest:   13 discovered,  13 passed, 0 skipped
failures:       0
errors:         0
