...
def normalize_ko(value: str) -> str:
    ...
    if not (2 <= len(normalized) <= 80):
        raise LookupFailure("KO must contain 2-80 letters, digits, spaces, dots, or hyphens", 2)
    if not normalized[0].isalnum() or any(
        not (char.isalpha() or char.isdigit() or char in " .-") for char in normalized
    ):
        raise LookupFailure("KO must contain 2-80 letters, digits, spaces, dots, or hyphens", 2)
    return normalized
...
def normalize_parcel(value: str) -> str:
    ...
    if not PARCEL_RE.fullmatch(normalized):
        raise LookupFailure("parcel must be digits, optionally followed by / and digits", 2)
    return normalized
...
def validate_geometry(geometry: Any) -> None:
    ...
    if len(positions) < 4:
        raise LookupFailure("RGZ returned an empty or incomplete parcel geometry", 5)
...
def fetch(url: str, timeout: float) -> bytes:
    ...
    try:
        ...
        raise LookupFailure(f"RGZ request failed: {exc.reason if isinstance(exc, URLError) else exc}", 5) from exc
...
def main() -> int:
    ...
    try:
        ...
        output = ROOT / "out" / f"{output_slug(ko)}-{parcel.replace('/', '-')}.geojson"
